...
function sendHtmlEmail($to, $subject, $htmlBody, $pdo = null) {
    // Récupérer config SMTP si dispo
    $fromEmail = 'noreply@eduardo-desul-immobilier.fr';
    $fromName = 'Eduardo De Sul — eXp France';
    ...
    if ($pdo) {
        try {
            ...
            foreach ($tables as $table) {
                $check = $pdo->query("SHOW TABLES LIKE '$table'")->rowCount();
                if ($check > 0) {
                    $rows = $pdo->query("SELECT setting_key, setting_value FROM $table WHERE setting_key LIKE 'smtp_%' OR setting_key LIKE 'email_%'")->fetchAll(PDO::FETCH_KEY_PAIR);
                    if (!empty($rows)) {
                        $fromEmail = $rows['smtp_from_email'] ?? $rows['email_from'] ?? $rows['smtp_user'] ?? $fromEmail;
                        $fromName = $rows['smtp_from_name'] ?? $rows['email_from_name'] ?? $fromName;
                        break;
                    }
                }
            }
    ...
    $headers .= "X-Mailer: IMMO-LOCAL-CMS\r\n";
