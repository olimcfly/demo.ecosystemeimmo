...
class JournalController
{
    ...
    public const PROFILES = [
        'vendeur'       => ['label' => 'Vendeur',        'icon' => "\xF0\x9F\x8F\xA0", 'color' => '#e74c3c'],
        'acheteur'      => ['label' => 'Acheteur',       'icon' => "\xF0\x9F\x94\x91", 'color' => '#3498db'],
        'investisseur'  => ['label' => 'Investisseur',   'icon' => "\xF0\x9F\x93\x88", 'color' => '#2ecc71'],
        'primo'         => ['label' => 'Primo-accedant', 'icon' => "\xF0\x9F\x8C\xB1", 'color' => '#9b59b6'],
    ];
    ...
    /**
     * Liste filtree
     * Utilisee par le widget (par canal) et le hub (global)
     */
    ...
    public function getConfig(): array
    {
        $config = [];
        try {
            $rows = $this->db->query("SELECT config_key, config_value FROM editorial_journal_config")->fetchAll(PDO::FETCH_ASSOC);
            foreach ($rows as $r) {
