...
if ($action === 'list') {
    $stmt = $pdo->query("SELECT * FROM secteurs ORDER BY name ASC");
    return ['success'=>true,'secteurs'=>$stmt->fetchAll()];
}

if ($action === 'get') {
    $stmt = $pdo->prepare("SELECT * FROM secteurs WHERE id=?"); $stmt->execute([(int)($p['id']??0)]);
    $row = $stmt->fetch();
...
if ($action === 'save' && $method === 'POST') {
    ...
    $pdo->prepare("INSERT INTO secteurs (`".implode('`,`',$cols)."`) VALUES (".implode(',',array_fill(0,count($cols),'?')).")")->execute(array_values($fields));
